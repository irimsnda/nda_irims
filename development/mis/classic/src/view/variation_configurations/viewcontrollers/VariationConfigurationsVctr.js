...
Ext.define('Admin.view.variation_configurations.viewcontrollers.VariationConfigurationsVctr', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.variationconfigurationsvctr',
    ...
    doCreateConfigParamWin: function (btn) {
        var me = this,
            ...
            storeID = btn.storeID,
            hasformFilter = btn.hasformFilter,
            store = Ext.getStore(storeID);
            ...
            frm.submit({
                ...
                success: function (form, action) {
                    ...
                    if (success == true || success === true) {
                        ...
                        if(hasformFilter){
                           // extraParams
                           //get the folder filters  par_variation_description
                           var formValues  = frm.getValues();
                           if(table_name == 'par_variation_categories'){
                              filters = {'t1.product_category_id' : formValues.product_category_id}
                              
                              filters = JSON.stringify(filters);
                           }else if(table_name == 'par_variation_subcategories'){
                                filters = {'t1.variation_category_id' : formValues.variation_category_id}
                                
                                filters = JSON.stringify(filters);
                            }else if(table_name == 'par_variation_description'){
                                filters = {'t1.variation_subcategory_id' : formValues.variation_subcategory_id}
                                
                                filters = JSON.stringify(filters);
                            }
                            else if(table_name == 'par_variationconditions_details' || table_name == 'par_variationsupporting_datadocs'){
                                filters = {'t1.variation_category_id' : formValues.variation_category_id}
                                
                                filters = JSON.stringify(filters);
                            }else if(table_name == 'par_variation_subdescription'){
                                filters = {'t1.variation_description_id' : formValues.variation_description_id}
                                
                                filters = JSON.stringify(filters);
                            }
                            
                            
                            //par_variation_description
                            else{
                                delete formValues.name;
                                delete formValues.description;
                                delete formValues.code;
                                delete formValues.model;
                                delete formValues.table_name;
                                delete formValues._token;
                                delete formValues.id;
                                filters = JSON.stringify(formValues);
                            }
