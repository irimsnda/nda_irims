...
@Component({
  selector: 'app-co-packedproduct-details',
  templateUrl: './co-packedproduct-details.component.html',
  styleUrls: ['./co-packedproduct-details.component.css']
})
export class CoPackedproductDetailsComponent extends SharedProductregistrationclassComponent{
 ...
 // @Input() summaryProductDetailsFrm:FormGroup;
  ...
  isCopackedDetailsWinshow:boolean=false;
 ...
 ngOnInit() { this.onLoadconfirmDataParmAll();
    ...
    this.productcopackedDetailsFrm = new FormGroup({
      ...
      therapeutic_group: new FormControl('', Validators.compose([Validators.required])),
      atc_code_description: new FormControl('', Validators.compose([Validators.required])),
...
onLoadATCCodesData(common_name_id) {
  ...
  onProdCopackedPreparing(e) {
    this.functDataGridToolbar(e, this.funcAddCopackedDetails, 'Product Other Details for Co packed products');
  }
  ...
  funcAddCopackedDetails() {
    
    this.isCopackedDetailsWinshow = true;
    this.productcopackedDetailsFrm.reset();
            ...
            .subscribe(
              data_response => {
                ...
                if (resp.success) {
                  ...
                  else if (reload_type == 'copacked_products') {
                    this.OnLoadProductsCommonName(appproduct_id);
...
OnLoadProductsCommonName(product_id) {
  ...
  this.appService.getProductsOtherDetails({ product_id: product_id }, 'getProductsCommonName')
    ...
    .subscribe(
      data => {
        if (data.success) {
          this.drugscopackedData = data.data;
        }
        else {
          this.toastr.success(data.message, 'Alert');
        }
...
onSaveProductCopackedDetails() {
  let appproduct_id = this.product_id;
  this.spinner.show();
  this.appService.onSaveProductOtherDetails('wb_copacked_products', this.productcopackedDetailsFrm.value, this.product_id)
    .subscribe(
      response => {
        this.product_resp = response.json();
        //the details 
        if (this.product_resp.success) {
          this.OnLoadProductsCommonName(appproduct_id);
          this.isCopackedDetailsWinshow = false;
          this.toastr.success(this.product_resp.message, 'Response');
        } else {
          this.toastr.error(this.product_resp.message, 'Alert');
        }
